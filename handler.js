...
if (user) {
    ...
    if (!('robo' in user))
        user.robo = 0
    if (!('robodurability' in user))
        user.robodurability = 0
    if (!('armor' in user))
        user.armor = 0
    if (!('armordurability' in user))
        user.armordurability = 0
    if (!('sword' in user))
        user.sword = 0
    if (!('sworddurability' in user))
        user.sworddurability = 0
    if (!('pickaxe' in user))
        user.pickaxe = 0
    if (!('pickaxedurability' in user))
        user.pickaxedurability = 0
    if (!('fishingrod' in user))
        user.fishingrod = 0
    if (!('fishingroddurability' in user))
        user.fishingroddurability = 0

    if (!('premium' in user))
        user.premium = false
    if (!('premiumTime' in user))
        user.premiumTime = 0
    if (!('joinlimit' in user))
        user.joinlimit = 0
...
if (chat) {
    ...
    if (!('simi' in chat))
        chat.simi = false
    if (!('expired' in chat)) // Menggunakan 'in' untuk memeriksa keberadaan properti
        chat.expired = 0
} else
    global.db.data.chats[m.chat] = {
        isBanned: false,
